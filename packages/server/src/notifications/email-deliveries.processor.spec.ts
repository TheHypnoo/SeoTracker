...
import { EmailDeliveriesProcessor } from './email-deliveries.processor';
...
describe('emailDeliveriesProcessor', () => {
  const notificationsService = { processEmailDelivery: jest.fn() };
  const configService = {
  ...
  it('creates an email worker and processes successful jobs', async () => {
    ...
    expect(notificationsService.processEmailDelivery).toHaveBeenCalledWith('delivery-1');
  ...
  it('records failed metrics and rethrows processing errors', async () => {
    notificationsService.processEmailDelivery.mockRejectedValueOnce(new Error('boom'));
    const processor = new EmailDeliveriesProcessor(
      notificationsService as never,
      configService as never,
      jobFailuresService as never,
