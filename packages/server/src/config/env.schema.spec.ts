...
import { apiEnvSchema, commonEnvSchema, workerEnvSchema } from './env.schema';
...
describe('apiEnvSchema', () => {
  it('parses a minimal valid environment with sensible defaults', () => {
    const env = apiEnvSchema.parse(BASE_ENV);
  ...
  it('rejects placeholder JWT secrets from .env.example', () => {
    expect(() =>
      apiEnvSchema.parse({ ...BASE_ENV, JWT_ACCESS_SECRET: `__replace_me__${'x'.repeat(40)}` }),
  ...
  it('rejects JWT secrets shorter than 32 chars', () => {
    expect(() => apiEnvSchema.parse({ ...BASE_ENV, JWT_ACCESS_SECRET: 'too-short' })).toThrow(
  ...
  it('coerces COOKIE_SECURE from "true"/"false" strings', () => {
    const env1 = apiEnvSchema.parse({ ...BASE_ENV, COOKIE_SECURE: 'true' });
    expect(env1.COOKIE_SECURE).toBe(true);

    const env2 = apiEnvSchema.parse({ ...BASE_ENV, COOKIE_SECURE: 'false' });
  ...
  it('rejects AUDIT_MAX_DEPTH outside [1, 3]', () => {
    expect(() => apiEnvSchema.parse({ ...BASE_ENV, AUDIT_MAX_DEPTH: '5' })).toThrow(/Too big/);
    expect(() => apiEnvSchema.parse({ ...BASE_ENV, AUDIT_MAX_DEPTH: '0' })).toThrow(/Too small/);
  });

  it('oTEL_ENABLED is false by default and accepts "true" string', () => {
    expect(apiEnvSchema.parse(BASE_ENV).OTEL_ENABLED).toBe(false);
    expect(apiEnvSchema.parse({ ...BASE_ENV, OTEL_ENABLED: 'true' }).OTEL_ENABLED).toBe(true);
  });

  it('aLERT_WEBHOOK_URL is optional but rejects malformed URLs', () => {
    expect(apiEnvSchema.parse(BASE_ENV).ALERT_WEBHOOK_URL).toBeUndefined();
    expect(() => apiEnvSchema.parse({ ...BASE_ENV, ALERT_WEBHOOK_URL: 'not-a-url' })).toThrow(
      /Invalid URL/,
    );
  });
});
...
describe('workerEnvSchema', () => {
  it('parses a minimal valid environment with worker defaults', () => {
    const env = workerEnvSchema.parse(BASE_ENV);
  ...
  it.each([
    ...
    const env = workerEnvSchema.parse(BASE_ENV) as Record<string, unknown>;
    expect(env[key]).toBeUndefined();
  });

  it('reuses the placeholder + length JWT validations from the common schema', () => {
    expect(() =>
      workerEnvSchema.parse({ ...BASE_ENV, JWT_ACCESS_SECRET: `__replace_me__${'x'.repeat(40)}` }),
    ).toThrow(/JWT_ACCESS_SECRET still uses the placeholder value/);
    expect(() => workerEnvSchema.parse({ ...BASE_ENV, JWT_ACCESS_SECRET: 'too-short' })).toThrow(
      /JWT_ACCESS_SECRET must be at least 32 chars/,
    );
  });
});
...
describe('commonEnvSchema', () => {
  it('omits service-specific fields (no PORT, no JOBS_HTTP_PORT)', () => {
    const env = commonEnvSchema.parse(BASE_ENV) as Record<string, unknown>;
