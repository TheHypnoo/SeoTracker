...
describe('envSchema', () => {
  it('parses a minimal valid environment with sensible defaults', () => {
    const env = envConfig.envSchema.parse(BASE_ENV);
  ...
  it('rejects JWT secrets shorter than 32 chars', () => {
    expect(() =>
      envConfig.envSchema.parse({ ...BASE_ENV, JWT_ACCESS_SECRET: 'too-short' }),
  ...
  it('coerces COOKIE_SECURE from "true"/"false" strings', () => {
    const env1 = envConfig.envSchema.parse({ ...BASE_ENV, COOKIE_SECURE: 'true' });
    expect(env1.COOKIE_SECURE).toBe(true);

    const env2 = envConfig.envSchema.parse({ ...BASE_ENV, COOKIE_SECURE: 'false' });
    expect(env2.COOKIE_SECURE).toBe(false);
  });

  it('rejects an invalid DATABASE_URL', () => {
    expect(() => envConfig.envSchema.parse({ ...BASE_ENV, DATABASE_URL: 'not-a-url' })).toThrow(
      /Invalid URL/,
    );
  });

  it('rejects AUDIT_MAX_DEPTH outside [1, 3]', () => {
    expect(() => envConfig.envSchema.parse({ ...BASE_ENV, AUDIT_MAX_DEPTH: '5' })).toThrow(
      /Too big/,
    );
    expect(() => envConfig.envSchema.parse({ ...BASE_ENV, AUDIT_MAX_DEPTH: '0' })).toThrow(
      /Too small/,
    );
  });
  ...
  it('oTEL_ENABLED is false by default and accepts "true" string', () => {
    expect(envConfig.envSchema.parse(BASE_ENV).OTEL_ENABLED).toBe(false);
    expect(envConfig.envSchema.parse({ ...BASE_ENV, OTEL_ENABLED: 'true' }).OTEL_ENABLED).toBe(
      true,
    );
  });

  it('aLERT_WEBHOOK_URL is optional but rejects malformed URLs', () => {
    expect(envConfig.envSchema.parse(BASE_ENV).ALERT_WEBHOOK_URL).toBeUndefined();
    expect(() =>
      envConfig.envSchema.parse({ ...BASE_ENV, ALERT_WEBHOOK_URL: 'not-a-url' }),
    ).toThrow(/Invalid URL/);
  });
