...
describe('startWorkerHttpServer', () => {
  ...
  it('serves liveness, readiness, metrics and fallback HTTP responses', async () => {
    ...
    try {
      await expect(request(port, '/health/liveness').then((res) => res.json())).resolves.toEqual({
        service: 'jobs',
        status: 'ok',
      });

      await expect(request(port, '/health/readiness').then((res) => res.json())).resolves.toEqual({
