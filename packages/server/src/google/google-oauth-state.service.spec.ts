...
describe('google oauth state service', () => {
  ...
  it('creates and verifies a signed OAuth state payload', () => {
    const state = service.create({ projectId: 'project-1', userId: 'user-1' }, secret);

    const payload = service.verify(state, secret);

    expect(payload.projectId).toBe('project-1');
    expect(payload.userId).toBe('user-1');
    expect(payload.nonce).toStrictEqual(expect.any(String));
  ...
  it('rejects tampered and expired states', () => {
    ...
    const expired = service.create(
      { projectId: 'project-1', ttlMs: -1000, userId: 'user-1' },
      secret,
    );
