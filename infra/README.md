...
# or the dev-only file if the external volume already exists:
...
- Run `pnpm db:migrate` during deploy before scaling API replicas. The API also runs pending migrations on bootstrap as a safety net, but that should not replace a controlled migration step.
